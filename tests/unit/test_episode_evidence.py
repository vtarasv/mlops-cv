...
class FakeRun:
    def __init__(self, run_id: str, experiment_id: str, tags: dict[str, str]) -> None:
        self.info = SimpleNamespace(run_id=run_id, experiment_id=experiment_id, status="RUNNING")
        self.data = SimpleNamespace(tags=dict(tags), metrics={})


class FakeClient:
    """A stand-in for ``MlflowClient``, recording every write."""

    def __init__(self) -> None:
        self.runs: dict[str, FakeRun] = {TRAINING_RUN: FakeRun(TRAINING_RUN, EXPERIMENT, {})}
    ...
    def search_runs(self, experiment_ids: list[str], filter_string: str = "") -> list[FakeRun]:
    ...
    def create_run(self, experiment_id: str, tags: dict[str, str], run_name: str) -> FakeRun:
        self._counter += 1
        run_id = f"record-{self._counter}"
        run = FakeRun(run_id, experiment_id, {**tags, "mlflow.runName": run_name})
...
def test_the_record_is_a_child_of_the_champions_training_run(client: FakeClient) -> None:
    ...
    assert record.info.experiment_id == EXPERIMENT  # a nested run lives in its parent's


def test_the_record_names_the_version_it_judges_not_the_alias(client: FakeClient) -> None:
...
def test_the_record_is_terminated_not_left_open(client: FakeClient) -> None:
...
def test_a_second_monitor_reuses_the_record_rather_than_duplicating_it(client: FakeClient) -> None:
...
def test_the_optimization_record_beside_it_is_not_mistaken_for_the_monitoring_one(
    ...
    sibling_tags = {PARENT_TAG: TRAINING_RUN, "optimize.model": "models:/x@champion"}
    client.runs["optimize-run"] = FakeRun("optimize-run", EXPERIMENT, sibling_tags)
    recorder = open_record(_resolved(), client=client)
...
def test_a_promotion_reparents_the_evidence(client: FakeClient) -> None:
    """Resolution ran against whoever is champion now, so the restart follows the promotion."""
    client.runs["train-run-2"] = FakeRun("train-run-2", EXPERIMENT, {})
...
def test_an_episode_is_exactly_one_step(client: FakeClient) -> None:
    recorder = open_record(_resolved(), client=client)
    recorder(_window())
...
def test_episodes_advance_the_step_within_one_monitor(client: FakeClient) -> None:
    recorder = open_record(_resolved(), client=client)
    recorder(_window())
    recorder(_window())
...
def test_a_restarted_monitor_continues_the_series(client: FakeClient) -> None:
...
def test_a_fresh_record_starts_at_step_zero(client: FakeClient) -> None:
...
def test_the_frames_are_recorded_by_address(client: FakeClient) -> None:
    recorder = open_record(_resolved(), client=client)
    recorder(_window())
...
def test_each_episode_gets_its_own_artifact(client: FakeClient) -> None:
    recorder = open_record(_resolved(), client=client)
    recorder(_window())
    recorder(_window())
...
def test_a_failed_write_costs_its_own_step_not_the_next_one(client: FakeClient) -> None:
    ...
    client.log_text = FakeClient.log_text.__get__(client)  # type: ignore[method-assign]
    recorder(_window())
...
def test_the_recorder_needs_no_open_run(client: FakeClient) -> None:
    ...
    client.runs["record-x"] = FakeRun("record-x", EXPERIMENT, {})
